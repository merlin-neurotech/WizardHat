...
def epoching_alt(data, samples_epoch, samples_overlap=0):
    n_epochs = n_samples // samples_epoch
    epoched_samples = n_epochs * samples_epoch
    data_, remainder = np.split(data, np.array([epoched_samples]))
...
def calc_fft(samples, sfreq, real_out=True):
    ...
    n_fft = int(np.floor(np.log2(n_samples)))
...
def calc_feature_vector(samples, sfreq, bands=BANDS):
